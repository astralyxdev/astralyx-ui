...
type AreaKey = 'ui' | 'primitives' | 'lib'

type Config = {
  tsx: boolean
  tailwind: { css: string }
  aliases: Record<AreaKey, string>
  paths: Record<AreaKey | 'styles', string>
}
...
const paint = (code: string) => (text: string) =>
  tty ? `${ESC}[${code}m${text}${ESC}[0m` : text
...
const info = (text: string) => log(`${cyan('.')} ${text}`)
const done = (text: string) => log(`${green('+')} ${text}`)
const warn = (text: string) => log(`${yellow('!')} ${text}`)
...
function readConfig(cwd: string): Config | undefined {
...
function requireConfig(cwd: string): Config {
  const config = readConfig(cwd)
  if (!config) fail(`No ${CONFIG_FILE} here. Run ${bold('astralyx-ui init')} first.`)
  return config
}
...
function targetFor(file: RegistryFile, config: Config, cwd: string) {
...
function installDeps(deps: string[], cwd: string, dry: boolean) {
...
function missingDeps(deps: string[], cwd: string) {
  const file = path.join(cwd, 'package.json')
  if (!fs.existsSync(file)) return deps
...
async function confirm(question: string, fallback: boolean) {
...
async function addItems(names: string[], config: Config, flags: Flags, cwd: string) {
  ...
  for (const file of written) done(file)
  if (skipped.length) {
  ...
  if (deps.length && !flags.noDeps) installDeps(deps, cwd, flags.dryRun)

  return { written, skipped }
}

async function init(flags: Flags, cwd: string) {
...
async function add(names: string[], flags: Flags, cwd: string) {
  ...
  if (wanted.length === 0) fail(`Nothing to add. Try ${bold('astralyx-ui list')}.`)
...
async function show(name: string, flags: Flags) {
  ...
  for (const file of files) log(dim(`  ${file.path}`))
...
function parse(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (arg === '--overwrite' || arg === '-o') flags.overwrite = true
    else if (arg === '--all') flags.all = true
    else if (arg === '--dry-run') flags.dryRun = true
    else if (arg === '--no-deps') flags.noDeps = true
    else if (arg === '--cwd') flags.cwd = argv[++i]
    else if (arg === '--registry') flags.registry = argv[++i]
    ...
    else if (/^--?(h|help|v|version)$/.test(arg)) rest.push(arg)
...
async function main() {
  ...
  if (flags.dryRun) warn('Dry run - nothing will be written.')
  ...
  switch (command) {
    ...
    case 'info':
    case 'show':
      if (!args[0]) fail('Which component? e.g. astralyx-ui info button')
